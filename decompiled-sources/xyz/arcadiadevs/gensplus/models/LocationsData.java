    ...
    public static class GeneratorLocation {
        ...
        public GeneratorLocation(String string2, String string3, Integer n, List<?> list) {
            block9: {
                ...
                List<String> list3 = ((List)map2.get("hologramLines")).isEmpty() ? GensPlus.getInstance().getConfig().getStringList(Config.DEFAULT_HOLOGRAM_LINES.getPath()) : (List<String>)map2.get("hologramLines");
        ...
        public GeneratorsData.Generator getGeneratorObject() {
            return GensPlus.getInstance().getGeneratorsData().getGenerator(this.generator);
        }
        ...
        public void spawn() {
            ...
            long l2 = TimeUtil.parseTime(GensPlus.getInstance().getConfig().getString(Config.ITEM_DESPAWN_TIME.getPath()));
            Bukkit.getScheduler().runTaskLater((Plugin)GensPlus.getInstance(), () -> this.getWorld().getEntities().stream().filter(entity -> entity instanceof Item).filter(arrayList::contains).forEach(Entity::remove), l2);
        }
