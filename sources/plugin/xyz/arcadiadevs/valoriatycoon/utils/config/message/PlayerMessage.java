    ...
    public void sendAsJson(Player player) {
        for (String string : this.format) {
            try {
                ...
                ValoriaTycoon.getInstance().getLogger().log(Level.WARNING, "Impossible d'analyser le message brut envoyé au joueur. Vérifiez que la syntaxe est correcte");
                ValoriaTycoon.getInstance().getLogger().log(Level.WARNING, "Message : " + string);
