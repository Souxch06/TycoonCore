...
public class SellUtil {
    public static void sellAll(Player player, Inventory inventory, boolean ... blArray) {
        ...
        while (n2 < inventory.getSize()) {
            ...
            if (itemStack != null) {
                if (NBTEditor.contains(itemStack, new Object[]{NBTEditor.CUSTOM_DATA, "gensplus", "spawnitem", "tier"})) {
                    int n3 = NBTEditor.getInt(itemStack, new Object[]{NBTEditor.CUSTOM_DATA, "gensplus", "spawnitem", "tier"});
                    GeneratorsData generatorsData = GensPlus.getInstance().getGeneratorsData();
        ...
        Economy economy = GensPlus.getInstance().getEcon();
    ...
    public static void sellWand(Player player, Inventory inventory, double d, WandData.Wand wand) {
        ...
        while (n2 < inventory.getSize()) {
            ItemStack itemStack = inventory.getItem(n2);
            if (itemStack != null && NBTEditor.contains(itemStack, new Object[]{NBTEditor.CUSTOM_DATA, "gensplus", "spawnitem", "tier"})) {
                int n3 = NBTEditor.getInt(itemStack, new Object[]{NBTEditor.CUSTOM_DATA, "gensplus", "spawnitem", "tier"});
                GeneratorsData generatorsData = GensPlus.getInstance().getGeneratorsData();
                GeneratorsData.Generator generator = generatorsData.getGenerator(n3);
    ...
    public static void sellHand(Player player) {
        ...
        if (!NBTEditor.contains(itemStack, new Object[]{NBTEditor.CUSTOM_DATA, "gensplus", "spawnitem", "tier"}) || bl) {
            Messages.NOTHING_TO_SELL.format(new Object[0]).send((CommandSender)player);
            return;
        }
        int n = NBTEditor.getInt(itemStack, new Object[]{NBTEditor.CUSTOM_DATA, "gensplus", "spawnitem", "tier"});
        GeneratorsData generatorsData = GensPlus.getInstance().getGeneratorsData();
