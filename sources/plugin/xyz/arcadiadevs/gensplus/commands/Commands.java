    ...
    public boolean onCommand(@NotNull CommandSender commandSender, @NotNull Command command, @NotNull String string, @NotNull String[] stringArray) {
        ...
        if (command.getName().equalsIgnoreCase("gensplus")) {
            ...
            if (stringArray.length == 0) {
                Messages.DEFAULT_MESSAGE.format("version", GensPlus.getInstance().getDescription().getVersion()).send(commandSender);
                return true;
            }
        ...
        if (command.getName().equalsIgnoreCase("selldrops")) {
            ...
            object = (Player)commandSender;
            if (!GensPlus.getInstance().getConfig().getBoolean(Config.SELL_COMMAND_ENABLED.getPath())) {
                return true;
            }
            if (stringArray.length == 0) {
                Messages.NOT_ENOUGH_ARGUMENTS.format(new Object[0]).send(commandSender);
