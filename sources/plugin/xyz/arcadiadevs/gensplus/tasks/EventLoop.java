...
extends BukkitRunnable {
    ...
    public void run() {
        if (nextEvent != null && nextEvent.startTime() < System.currentTimeMillis()) {
            ...
            Messages.EVENT_STARTED.format("event", activeEvent.event().getName(), "time", TimeUtil.millisToTime(eventDuration)).send(GensPlus.getInstance().getConfig().getBoolean(Config.EVENTS_BROADCAST_ENABLED.getPath()));
            nextEvent = null;
            return;
        }
        if (activeEvent.endTime() < System.currentTimeMillis()) {
            Messages.EVENT_ENDED.format("event", activeEvent.event().getName(), "time", TimeUtil.millisToTime(timeBetweenEvents)).send(GensPlus.getInstance().getConfig().getBoolean(Config.EVENTS_BROADCAST_ENABLED.getPath()));
            activeEvent = new ActiveEvent(null, System.currentTimeMillis(), System.currentTimeMillis() + timeBetweenEvents);
            EventLoop.setRandomNextEvent();
        }
    }
