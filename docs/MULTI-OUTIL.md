...
| survit à la mort | l'outil est **retiré des drops** de `PlayerDeathEvent` (la liste vivante de `getDrops()`), donc il ne tombe pas avec le reste — sans jamais toucher à `keepInventory` du serveur, qui est une règle globale | un mort en fer = un joueur sans outil, à retarter au respawn |
